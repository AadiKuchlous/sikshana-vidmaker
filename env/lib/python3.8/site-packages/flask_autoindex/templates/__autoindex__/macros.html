{% macro entry(ent) %}
  <tr>
    ...
    {% if not ent.size %}
    {% if not ent.name == ".." %}
        <img onClick='show_delete_popup("{{ ent.name }}")' src="{{ url_for('__autoindex__.static', filename='delete.png') }}" width="25px" style="float:right; margin-left: 12px;">
	<img onClick='show_rename_popup("{{ ent.name }}")' src="{{ url_for('__autoindex__.static', filename='pencil-square.svg') }}" width="25px" style="float:right;"></button>
    {% endif %}
    {% endif %}
    </td>
    <td class="modified">
